using System;
using System.Diagnostics;

o2cW

namespace Model {
    public class Simulation {
        private uint AngleOfImpact { get; set; }
        private float ImpactForce { get; set; }
        private float Velocity { get; set; }

        // Initialisation of the Class \\
        /// <summary>
        /// Initialiser for the Simulation class
        /// </summary>
        /// <param name="angleOfImpact"></param>
        /// <param name="ForceOnCar"></param>
        /// <param name="velocity"></param>
        public Simulation(uint angleOfImpact, float forceOnCar, float velocity) { // Sets up the class
            AngleOfImpact = angleOfImpact;
            ImpactForce = Math.Abs(forceOnCar);
            Velocity = Math.Abs(velocity);
        }

        // Starts the simulation \\
        /// <summary>
        /// Starts a simulation with the properties set
        /// </summary>
        /// <returns>
        /// Chance of a collision 0 -> 1
        /// </returns>
        public float Start() {
            if (Velocity == 0) {
                Velocity = 1; // Sanity checking the velocity so we don't divide by zero
            }

            var proc = new Process(); // Sets up the process for the python file
            proc.StartInfo.FileName = @"py.exe";
            proc.StartInfo.Arguments = $"Detection.py {ImpactForce} {Velocity} {AngleOfImpact}"; 
            proc.StartInfo.UseShellExecute = false;
            proc.StartInfo.RedirectStandardOutput = true;
            proc.StartInfo.RedirectStandardError = true;
            proc.Start(); // Runs the python file

            string output = proc.StandardOutput.ReadToEnd();

            float collisionChance;
            float.TryParse(output, out collisionChance); // Tries to convert the output from the simulation to a float

            return collisionChance;
        }
    }
}
